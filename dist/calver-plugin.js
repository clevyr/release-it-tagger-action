...
module.exports = function createSemanticVersion(format, ver, alltags) {
  ...
  function inc(level) {
    if (['DEV', 'ALPHA', 'BETA', 'RC'].indexOf(level) !== -1) {
      ...
      semversion.MODIFIER = level.toLowerCase() + '.' +
        (semversion.MODIFIER == 'modifier.0' ? '0' :
        (isLevelChanged ? '0' : (parseInt(mv) + 1)))
    }
  ...
  function asString() {
    return Object.keys(semversion)
      ...
      + (semversion.MODIFIER ? '-' + semversion.MODIFIER : '')
  }
...
const createDateVersion = __nccwpck_require__(600)
const createSemanticVersion = __nccwpck_require__(547)
...
module.exports = __nccwpck_require__(868)(exports);
...
function setup(env) {
	...
	createDebug.humanize = __nccwpck_require__(964);
...
try {
	...
	const supportsColor = __nccwpck_require__(819);
...
const debug = __nccwpck_require__(763);
const _ = __nccwpck_require__(380);
...
const tty = __nccwpck_require__(867);
const hasFlag = __nccwpck_require__(303);
...
const Plugin = __nccwpck_require__(465),
    calver = __nccwpck_require__(305),
